    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-black text-slate-900">Produksi Susu Sapi</h1>
          <p className="text-xs text-slate-500 font-medium">
            {canManage ? 'Input dan kelola volume perah susu sapi harian dari seluruh kandang.' : 'Lihat informasi hasil perah susu sapi harian.'}
          </p>
        </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex items-center justify-between">
          <h2 className="text-base font-black text-slate-800 flex items-center gap-2">
            ...
            <span>Tabel Data Produksi Susu Sapi ({filteredProductions.length} Entry)</span>
          </h2>
        </div>
            ...
            <div className="bg-white rounded-3xl max-w-md w-full p-6 space-y-5 shadow-2xl animate-in fade-in zoom-in duration-200">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                <h3 className="font-black text-slate-900 text-base flex items-center gap-2">
                  ...
                  <span>{editingProd ? 'Edit Data Produksi' : 'Input Produksi Susu Sapi'}</span>
                </h3>
                <button onClick={() => setShowModal(false)} className="text-slate-400 hover:text-slate-600 text-lg">✕</button>
              </div>

              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="p-3 bg-emerald-50/70 rounded-2xl border border-emerald-200/70 flex items-center gap-2.5">
                  <span className="text-xl">🐄</span>
                  <div>
                    <span className="font-black text-emerald-950 text-xs block">Susu Murni Sapi</span>
                    <span className="text-[11px] text-emerald-700 font-semibold">Pencatatan hasil perah kandang sapi</span>
                  </div>
                </div>
