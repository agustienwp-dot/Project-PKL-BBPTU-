...
export default function PengemasanPage() {
  ...
  const handleExportExcel = () => {
    if (filteredPackagings.length === 0) {
      setToast({ type: 'error', message: 'Tidak ada data pengemasan untuk diexport.' });
      return;
    }

    const dataToExport = filteredPackagings.map((p, idx) => {
      ...
      if (p.packagingDetails) {
        try {
          const parsed = JSON.parse(p.packagingDetails);
          if (Array.isArray(parsed) && parsed.length > 0) {
            detailsStr = parsed.map(it => `${it.packagingType} (${it.size || '-'}): ${it.quantity} pcs`).join('; ');
          }
        } catch (e) {}
      }
      if (!detailsStr) {
        detailsStr = `${p.packagingType || 'Botol'} (${p.totalPackagedQty} pcs)`;
      }
      ...
      return {
        ...
        'Kategori Utama': pCat,
        'Produk / Subkategori': pSub,
        'Asal Bahan': pOri,
        'Varian / Rasa': pVar,
        'Bahan Diproses': `${p.processedAmount || p.processedLiters || 0} ${p.processedUnit || (pCat === 'Keju' ? 'Kg' : 'Liter')}`,
        'Rincian Kemasan': detailsStr,
        'Total Produk Jadi (pcs)': p.totalPackagedQty || 0,
        'Status Transfer': p.status || 'DRAFT',
        'Petugas Input': p.createdBy?.name || '-',
        'Catatan': p.notes || '-'
      };
    });

    exportToExcel(dataToExport, `Data_Hasil_Pengemasan_${new Date().toISOString().split('T')[0]}.xlsx`, 'Hasil Pengemasan');
    setToast({ type: 'success', message: 'Data hasil pengemasan berhasil diexport ke Excel!' });
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            title="Export data pengemasan ke file Excel"
          >
