...
export default function RequestSusuMasukPemasaranPage() {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case 'DISETUJUI':
        return <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold bg-blue-500/10 text-blue-600 border border-blue-500/20"><CheckCircle2 className="w-3.5 h-3.5" /> Disetujui (Perlu Diserahkan)</span>;
      case 'DIPROSES':
      case 'SIAP_DITERIMA':
        return <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold bg-teal-500/10 text-teal-600 border border-teal-500/20"><Truck className="w-3.5 h-3.5" /> Siap Diterima Pengemasan</span>;
      case 'DITERIMA':
        return <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold bg-emerald-500/10 text-emerald-600 border border-emerald-500/20"><CheckCircle2 className="w-3.5 h-3.5" /> Diterima Pengemasan</span>;
      case 'DITOLAK':
