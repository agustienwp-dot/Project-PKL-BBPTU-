...
function attachDelegates(client) {
  ...
  if (!client.bastDocument && client.beritaAcara) {
    client.bastDocument = client.beritaAcara;
  }
  return client;
}

// Reset global cached client if it failed or was created with invalid URL
if (!globalForPrisma.prisma || globalForPrisma._prismaUrl !== dbUrl || globalForPrisma._schemaVersion !== 2) {
  globalForPrisma.prisma = createClient();
  globalForPrisma._prismaUrl = dbUrl;
  globalForPrisma._schemaVersion = 2;
} else {
