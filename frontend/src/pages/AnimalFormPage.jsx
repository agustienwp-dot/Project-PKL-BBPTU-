...
export default function AnimalFormPage() {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        if (!isEdit && res.data.data.length > 0) {
          setFormData((prev) => ({ ...prev, cageId: res.data.data[0].id }));
        }
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let res;
      if (isEdit) {
        res = await api.put(`/animals/${id}`, payload);
      } else {
        res = await api.post('/animals', payload);
      }

      if (res.data.success) {
        setToast({ message: isEdit ? 'Data hewan berhasil diperbarui' : 'Hewan baru berhasil ditambahkan', type: 'success' });
        setTimeout(() => {
          navigate('/animals');
        }, 1200);
      }
      ...
      const msg = err.response?.data?.message || 'Gagal menyimpan data hewan';
  ...
  if (loading) return <LoadingSpinner text="Memuat form hewan..." />;
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-12">
      ...
      <div className="flex items-center gap-4">
        <Link to="/animals" className="p-2 bg-white hover:bg-slate-100 border border-slate-300 text-slate-600 rounded-xl transition-colors shadow-sm">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div>
          <h1 className="text-2xl font-black text-[#1E3F20]">{isEdit ? 'Edit Data Hewan Ternak' : 'Tambah Hewan Ternak Baru'}</h1>
          <p className="text-slate-600 text-xs mt-0.5">Isi seluruh informasi identitas, fisik, dan asal-usul hewan</p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full bg-[#F5F5F0] border border-slate-300 rounded-xl px-3.5 py-2.5 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-[#1E3F20] disabled:opacity-60"
              />
            </div>
